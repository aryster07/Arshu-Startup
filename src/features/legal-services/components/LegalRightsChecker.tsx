...
export default function LegalRightsChecker({ onNavigate }: LegalRightsCheckerProps) {
  ...
  const downloadCaseAnalysis = () => {
    ...
    const content = `
LAW BANDHU - LEGAL RIGHTS ANALYSIS
...
${analysisResult.rights.map((right, index) => 
  `${index + 1}. ${right.title} (${right.importance.toUpperCase()})
   ${right.description}`
).join('\n\n')}
...
${analysisResult.violations.length > 0 ? 
  analysisResult.violations.map((violation, index) => 
    `${index + 1}. ${violation.law} (${violation.severity.toUpperCase()})
   ${violation.description}`
  ).join('\n\n') : 'No specific violations identified.'}

==================================================
AVAILABLE REMEDIES
==================================================
${analysisResult.remedies.map((remedy, index) => 
  `${index + 1}. ${remedy.action} (${remedy.timeline})
   ${remedy.description}`
).join('\n\n')}
...
RISK ASSESSMENT
==================================================
Overall Risk: ${analysisResult.riskAssessment.overall.toUpperCase()}
Factors: ${analysisResult.riskAssessment.factors.join(', ')}
...
Generated by Law Bandhu - Your Legal Assistant
    `.trim();
    ...
    a.href = url;
    a.download = `Legal_Analysis_${new Date().toISOString().split('T')[0]}.txt`;
  ...
  const LegalAnalysisDisplay = ({ result }: { result: LegalAnalysisResult }) => (
    <div className="mt-6">
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="flex flex-wrap lg:flex-nowrap w-full h-auto gap-1">
          <TabsTrigger value="overview" className="text-xs lg:text-sm py-3 lg:py-2 flex-1 lg:flex-none">
            Overview
          </TabsTrigger>
          <TabsTrigger value="violations" className="text-xs lg:text-sm py-3 lg:py-2 flex-1 lg:flex-none">
            Violations
          </TabsTrigger>
          <TabsTrigger value="rights" className="text-xs lg:text-sm py-3 lg:py-2 flex-1 lg:flex-none">
            Rights
          </TabsTrigger>
          <TabsTrigger value="remedies" className="text-xs lg:text-sm py-3 lg:py-2 flex-1 lg:flex-none">
            Remedies
          </TabsTrigger>
          <TabsTrigger value="steps" className="text-xs lg:text-sm py-3 lg:py-2 flex-1 lg:flex-none">
            Next Steps
          </TabsTrigger>
        </TabsList>
          ...
          <Card className="p-6 bg-red-50 border-red-200">
            ...
            {result.violations.length > 0 ? (
                ...
                {result.violations.map((violation, index) => (
                  <div key={index} className="bg-white border border-red-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h5 className="font-semibold text-red-900">{violation.law}</h5>
                      <Badge 
                    ...
                    <p className="text-red-800 text-sm">{violation.description}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8">
                ...
                <p className="text-slate-600 text-lg font-medium">No Legal Violations Identified</p>
                <p className="text-slate-500 text-sm mt-2">The AI analysis did not identify any specific legal violations in your case. This could mean your situation may not involve legal violations, or you may need to provide more details.</p>
              </div>
            )}
          </Card>
        </TabsContent>
            ...
            {result.rights.length > 0 ? (
              <div className="space-y-4">
                {result.rights.map((right, index) => (
                  <div key={index} className="bg-white border-l-4 border-blue-500 rounded p-4">
                    <div className="flex items-center justify-between mb-2">
                    ...
                    <p className="text-slate-700">{right.description}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8">
                ...
                <p className="text-slate-600 text-lg font-medium">No Specific Rights Identified</p>
                <p className="text-slate-500 text-sm mt-2">The AI analysis couldn't identify specific legal rights for your case. Try providing more details about your situation for better analysis.</p>
              </div>
            )}
          </Card>
        </TabsContent>
              ...
              {result.remedies.length > 0 ? (
                result.remedies.map((remedy, index) => (
                  <div key={index} className="bg-white border border-green-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-green-900">{remedy.action}</h4>
                      <Badge variant="outline" className="text-xs border-green-300 text-green-700">
                        {remedy.timeline}
                      </Badge>
                    </div>
                    <p className="text-green-800">{remedy.description}</p>
                  </div>
                ))
              ) : (
                <div className="text-center py-8">
            ...
            {result.nextSteps.length > 0 ? (
              <ul className="space-y-3">
                {result.nextSteps.map((step, index) => (
                  <li key={index} className="flex items-start space-x-3 bg-white p-4 rounded border border-yellow-200">
                ...
                <div className="flex gap-4">
                  <div className="flex-1 relative">
                    ...
                    <button
                      ...
                      className={`absolute right-2 top-2 p-1.5 rounded-sm transition-colors ${
                        isListening 
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {legalCategories.map((category) => (
                    <button
                      key={category.id}
                      type="button"
                      className={`h-auto p-4 flex flex-col items-center space-y-2 text-center border-3 rounded-lg cursor-pointer transition-colors font-semibold ${
