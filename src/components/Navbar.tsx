    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          >
            <div className={`p-2.5 bg-slate-900 rounded-xl group-hover:bg-slate-800 transition-all duration-300 ${
              scrolled ? 'shadow-lg' : ''
            }`}>
          ...
          <div className="hidden md:flex items-center space-x-1">
            {mainNavItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`relative px-4 py-2.5 rounded-xl font-medium transition-all duration-300 hover:scale-105 active:scale-95 group ${
                  isActive(item.path)
              ...
              >
                ...
                <div className={`absolute inset-0 bg-gradient-to-r from-blue-50 to-slate-50 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${
            ...
            <div className="relative">
              <button
                ...
                className={`relative px-4 py-2.5 rounded-xl font-medium transition-all duration-300 hover:scale-105 active:scale-95 group flex items-center space-x-2 ${
                  isActive('/services') || isActive('/emergency')
                    ? 'text-blue-600 bg-blue-50'
                    : 'text-slate-700 hover:text-slate-900 hover:bg-slate-50'
                }`}
              >
                ...
                <div className={`absolute inset-0 bg-gradient-to-r from-blue-50 to-slate-50 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${
                  isActive('/services') || isActive('/emergency') ? 'opacity-100' : ''
                }`}></div>
              </button>
              
              {isServicesOpen && (
                <div className="absolute top-full left-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200/50 backdrop-blur-sm animate-in fade-in-0 zoom-in-95 duration-200">
                  <div className="p-6">
                    <div className="mb-4">
                      <h3 className="font-semibold text-slate-900 mb-1">Legal Services</h3>
                      <p className="text-sm text-slate-600">Comprehensive professional legal assistance</p>
                    </div>
                    <div className="space-y-2">
                      {serviceItems.map((service) => (
                        <Link
                          key={service.path}
                          to={service.path}
                          className="flex items-start space-x-3 p-3 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-slate-50 transition-all duration-200 hover:translate-x-1 group"
                          onClick={() => setIsServicesOpen(false)}
                        >
                          <div className="flex-shrink-0 w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center group-hover:bg-blue-100 transition-colors duration-200">
                            {service.icon}
                          </div>
                          <div className="flex-1 min-w-0">
                            <h4 className="font-medium text-slate-900 text-sm group-hover:text-blue-600 transition-colors duration-200">
                              {service.title}
                            </h4>
